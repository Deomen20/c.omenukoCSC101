fn main() {
    ...
    for count in 0..city_num {
        ...
        std::io::stdin().read_line(&mut input2).expect("Faileed to read input");
        let new_city:String = input2.trim().parse().expect("Invalis input");
        city.push(new_city);
    }
    ...
    for i in city
    {
        // iterating through i on th vector
        println!("{} {}", count, i);
        count+=1;
    }
}
